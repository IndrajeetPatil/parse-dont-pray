...
## Parse, Don't Pray {style="margin-top: 1em; margin-bottom: 2.5em;"}
...
![](media/social-media-card.jpg){width="100%" fig-alt="Parse, Don't Pray - Two hands clasped together symbolizing the partnership between Pydantic and Zod for data validation"}
...
Understanding the type system helps explain why data validation is crucial.
...
::: {style="background-color: #e8f5e9; padding: 20px; border-radius: 10px;"}
...
::: {style="background-color: #fff3e0; padding: 20px; border-radius: 10px;"}
...
::: {style="background-color: #e3f2fd; padding: 20px; border-radius: 10px; margin-top: 20px; text-align: center;"}

**Why validation matters more for dynamic languages**: Without compile-time checks, you need **runtime validation** to ensure data integrity. Validation libraries bridge this gap!

:::

## What is Data Validation? {.smaller}

Data validation is the process of ensuring that data meets specific criteria before it's used in your application.

. . .

::: {style="background-color: #fff3e0; padding: 20px; border-radius: 10px; margin: 20px 0;"}

**Without validation libraries**, you write manual checks scattered throughout your code:
...
**With validation libraries**, you define schemas that automatically validate, parse, and type your data.
...
::: {style="background-color: #ffffff; border: 2px solid #E92063; border-radius: 15px; padding: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); font-size: 0.85em;"}
...
- Integration with FastAPI, SQLModel, and more
- JSON Schema generation
- V2 offers 5-50x performance improvements

::: {style="text-align: center; margin-top: 10px; font-size: 0.95em;"}
...
::: {style="background-color: #ffffff; border: 2px solid #3E67B1; border-radius: 15px; padding: 20px; box-shadow: 0 4px 6px rgba(0,0,0,0.1); font-size: 0.85em;"}
...
- Zero dependencies, works in all JS environments
...
Validation rules serve as inline documentation, making it immediately clear what data structure is expected.
...
class UserProfile(BaseModel):
    """User profile with built-in validation."""
...
Schema definition, validation logic, and type information all live in one place instead of being scattered across the codebase.
...
Describes *what* the data should look like rather than *how* to validate it, reducing cognitive load.
...
**Result**: Reduced cognitive load—focus on business logic, not validation boilerplate!
:::

## Specific Types Over Generic {.smaller}

Using specific, domain-appropriate types dramatically improves code readability and catches errors early.
...
::: {style="background-color: #fff3e0; padding: 15px; border-radius: 8px;"}
...
**Another example: Model temperature constraints**
...
class LLMConfig(BaseModel):
    ...
    # Best: Self-documenting type
    temperature: confloat(ge=0.0, le=2.0)  # NonNegativeFloat constrained to [0.0, 2.0]
```
...
const LLMConfigSchema = z.object({
  ...
  // Best: Extracted for reuse
  temperature: z.number().min(0.0).max(2.0).brand('ModelTemperature'),
});
```
...
**Progression**: `str` → `EmailStr` → `AcmeEmailStr` = **Increasing specificity = Better readability**
:::
...
Data is validated as it enters your system, catching errors early before they propagate.
...
Automatically converts compatible types following sensible rules.
...
event = Event(
    event_id="42",  # String "42" → int 42
    timestamp="2024-01-15T10:30:00",  # String → datetime
    is_public="yes",  # String "yes" → bool True
    attendees=("Alice", "Bob")  # Tuple → list
)
...
const event = EventSchema.parse({
  eventId: "42",  // String "42" → number 42
  timestamp: "2024-01-15T10:30:00",  // String → Date
  isPublic: true,
  attendees: ["Alice", "Bob"],
});
...
Provides specific, actionable feedback about what's wrong and where.
...
if (!result.success) {
  ...
  //   "age": { "_errors": ["Number must be greater than or equal to 13"] },
...
::: {style="background-color: #fff3e0; padding: 15px; border-radius: 10px; margin-top: 15px; text-align: center;"}
**Developer-friendly**: Know exactly what's wrong, where it's wrong, and what's expected!
:::
...
TypeScript/Python types are automatically derived from your schemas—no duplicate type definitions needed.
...
Ensures runtime data actually matches your static types, bridging the gap between type systems and reality.
...
When you update the schema, types automatically update—impossible for them to become out of sync.
...
Don't need to write as many validation tests since the library handles edge cases.
...
Many libraries offer utilities to generate valid test data from schemas.
...
Schemas can be shared between frontend/backend or services to ensure compatibility.
...
Strips unknown/dangerous fields automatically, preventing mass assignment vulnerabilities.
...
Ensures attackers can't exploit loose type coercion in JavaScript/Python.
...
Enforce constraints on string lengths, number ranges, array sizes to prevent DoS attacks.
...
Auto-generate OpenAPI/Swagger specs directly from schemas, ensuring docs stay synchronized.
...
Built-in serialization/deserialization (e.g., date parsing, nested object handling).
...
raw_data = {
    "title": "My Blog Post",
    "published_at": "2024-01-15T10:30:00Z",  # String → datetime
...
post = BlogPost(
    ...
    published_at="2024-01-15T10:30:00",  # String → datetime object
...
const BlogPostSchema = z.object({
  ...
  tags: z.union([
    z.array(z.string()),
    z.string().transform(s => s.split(","))  // Parse "a,b,c" → ["a","b","c"]
  ]),
...
const rawData = {
  title: "My Blog Post",
  slug: "",
  publishedAt: "2024-01-15T10:30:00Z",  // String → Date object
  tags: "python,fastapi,pydantic",  // String → Array
};
...
For data that shouldn't change after creation, use frozen/immutable models to validate once and guarantee immutability.
...
- ✅ **Validate once at startup**, use everywhere without re-checking
- ✅ **Prevent accidental mutations** that could break application state
- ✅ **Thread-safe** - no risk of concurrent modifications
- ✅ **Clearer intent** - signals this data is meant to be constant
- ✅ **Performance** - no validation overhead after initial parse

:::

## Developer Experience {.smaller}

Reduces boilerplate and improves onboarding with cross-language consistency.
...
user = User(**data)  # ← One line replaces 50+ lines
```
...
// That's it! Full validation, type safety, IDE support
const user = UserSchema.parse(data);  // ← One line replaces 50+ lines
```
