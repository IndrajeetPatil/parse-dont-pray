...
class Event(BaseModel):
    ...
    is_public: bool      # <3>
    attendees: list[str] # <4>
```

1. Coerces string to `int`
2. Parses ISO 8601 strings to `datetime`
3. Coerces `"yes"` / `"no"` to `bool`
4. Coerces tuple to `list`
...
const EventSchema = z.object({
  eventId: z.coerce.number(),   // <1>
  timestamp: z.coerce.date(),   // <2>
  isPublic: z.boolean(),
  attendees: z.array(z.string()),
});
```

1. `z.coerce` converts string input to `number`
2. `z.coerce` parses string input to `Date`
...
1. Rejects any fields not defined in the schema
...
const UserUpdateSchema = z.strictObject({  // <1>
  email: z.string().email(),
});
```
...
**Result**: Privilege escalation and mass assignment risk.

**Lesson**: Client validation is for **UX**, not security. Matching schemas on both sides act as a **contract test** – catching API mismatches at runtime.
...
1. Submit form → wait 200ms
2. "Email invalid" → fix → wait 200ms
3. "Age too low" → fix → wait 200ms
4. Finally succeeds after 3 round trips

**Result**: Frustration, poor UX, abandoned forms.

**Lesson**: Server validation is **mandatory** for security, but without client validation, UX suffers.
...
config.max_retries = 5  # <2>
```

1. Makes all fields immutable after creation
2. 🚫 `ValidationError`: frozen instance

### TypeScript (Zod)

```{.typescript code-line-numbers="5,8"}
import { z } from "zod";
...
config.maxRetries = 5;  // <2>
```

1. Makes all fields `Readonly` after parsing
2. 🚫 Compile error: readonly property
